...
public class LevelPath : MonoBehaviour
{
    ...
    {
        GameObject terrainObject = GameObject.Find("Quad");
        TerrainMesh terrainMesh = null;

        if(terrainObject == null)
        {
            Debug.LogError("Terrain Object null.");
            return;
        }
        else
        {
            Debug.Log("Found " + terrainObject.name);
        }

        terrainMesh = terrainObject.GetComponent<TerrainMesh>();

        Vector3[] vertices = terrainMesh.GetVertices().ToArray();
        float[] heightMap = terrainMesh.GetHeightMap().ToArray();
        ...
        float hit = 0f;
         for (int i = 0; i < vertices.Length; ++i)
         {
            ...
            for(int j = 0; j < 100; ++j)
            {
                float interpolant = (float)j / (float)100;
                float distance = Vector3.Distance(vertices[i], GetPointOnSpline(interpolant));
                if(distance > 50)
                {
                    break;
                }
        ...
        {
            newHeightMap.Add(heightMap[i]);
        }
        terrainMesh.UpdateHeightMap(newHeightMap);
